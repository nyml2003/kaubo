...
class MatrixMultiply(Operator):
    ...
    def get_jacobi(self, parent):
        ...
        if parent is parent1:
            ...
            for i in range(m):
                for j in range(n):
                    jacobi[i * k : (i + 1) * k, j * k : (j + 1) * k] = (
                        Eye(k) * parent0.value[i, j]
                    )
            return jacobi
        ...
        for node in self.graph.nodes:
            if node.trainable:
                ...
                if node not in self.v:
                    ...
                    self.v[node] = (
                        self.v[node] * self.momentum - gradient * self.learning_rate
                    )
