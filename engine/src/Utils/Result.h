...
class Result {
 ...
 public:
  ...
  Result(OkType ok) requires IsNonVoid<T>
    : m_data(OkValue{std::move(ok.get())}) {}

  template <OkIntermediateFor<T> OkType>
  // NOLINTNEXTLINE(google-explicit-constructor)
  Result(OkType /*unused*/) requires IsVoid<T> : m_data(OkValue{}) {}
  ...
  [[nodiscard]] auto unwrap() const -> const U& requires IsNonVoid<U> {
  ...
  auto unwrap() const -> void requires IsVoid<T> {
    if (!is_ok()) {
      throw std::runtime_error("Called unwrap() on Err");
    }
  }
  ...
  template <typename U = T>
  [[nodiscard]] auto expect(const std::string& msg) const
    -> const U& requires IsNonVoid<U> {
    if (is_ok()) {
      return unwrap();
    }
  ...
  // T为void时的版本，无返回值
  auto expect(const std::string& msg) const -> void requires IsVoid<T> {
    if (!is_ok()) {
      throw std::runtime_error(msg);
    }
  }
  ...
  auto map(F&& f) const -> Result<U, E> requires IsNonVoid<U> {
    if (is_ok()) {
  ...
  [[nodiscard]] auto flatten() const
    -> Result<typename U::OkType, E> requires IsNestedResult<U, E> {
    if (is_err()) {
      return Err(unwrap_err());
    }
  ...
  template <typename U = T>
  [[nodiscard]] auto flatten() const
    -> Result<void, E> requires IsVoid<U>&& IsNestedResult<U, E> {
    static_assert(false, "Result<void, E> 无需 flatten（无嵌套 Ok 值）");
  }

  template <typename F, typename U = std::invoke_result_t<F, T>>
  [[nodiscard]] auto and_then(F&& f) const
    -> Result<typename U::OkType, E> requires(
      IsNestedResult<U, E>&& std::is_invocable_v<F, T>
    ) {
    if (is_err()) {
      return Err(unwrap_err());
    }
